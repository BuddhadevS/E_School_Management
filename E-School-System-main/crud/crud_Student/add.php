<style>
  body {background-image: linear-gradient(90deg , beige, #3e423b  );}
  </style>
<?php
include '../../pdo.php';
session_start();

echo('<form method="post">
<input type="submit" value="DashBoard" name="dashboard">
</form>');

if(isset($_POST['dashboard'])){
    if($_SESSION['user_type'] == "admin"){
        header( 'Location: http://localhost/e-school%20system/Admin/HomeIndexAdmin.php' ) ;
    }
else if($_SESSION['user_type'] == "student"){
        header( 'Location: http://localhost/e-school%20system/Student/HomeIndexStudent.php' ) ;
    }
else if($_SESSION['user_type'] == "teacher"){
        header( 'Location: http://localhost/e-school%20system/Parent/HomeIndexParent.php' ) ;
    }
else if($_SESSION['user_type'] == "teacher"){
        header( 'Location: http://localhost/e-school%20system/teacher/HomeIndexteacher.php' ) ;
    }
    else if($_SESSION['user_type'] == "library"){
        header( 'Location: http://localhost/e-school%20system/Library/HomeIndexLibraryAdmin.php' ) ;
    }
else{
        header( 'Location: http://localhost/e-school%20system/' ) ;
    }
}



if ( isset($_POST['Name']) && isset($_POST['s_ID'])
     && isset($_POST['password'])) {

    // Data validation
    if ( strlen($_POST['Name']) < 1 || strlen($_POST['password']) < 1) {
        $_SESSION['error'] = 'Missing data';
        header("Location: add.php");
        return;
    }

    $sql = "INSERT INTO student (s_ID, Name, Address, Contact, doB, doJ, passWord, current_Class, p_ID, email_Address, img_student)
                        VALUES (:s_ID, :Name, :Address, :Contact, :doB, :doJ, :passWord, :current_Class, :p_ID, :email_Address, :img_student)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(
        ':s_ID' => $_POST['s_ID'],
        ':Name' => $_POST['Name'],
        ':Address' => $_POST['Address'],
        ':Contact' => $_POST['Contact'],
        ':doB' => $_POST['doB'],
        ':doJ' => $_POST['doJ'],
        ':passWord' => $_POST['password'],
        ':email_Address' => $_POST['email_Address'],
        ':img_student' => $_POST['img_student'],
        ':current_Class' => $_POST['current_Class'],
        ':p_ID' => $_POST['p_ID']));
    $_SESSION['success'] = 'Record Added';
    header( 'Location: index.php' ) ;
    return;
}

// Flash pattern
if ( isset($_SESSION['error']) ) {
    echo '<p style="color:red">'.$_SESSION['error']."</p>\n";
    unset($_SESSION['error']);
}
?>
<p>Add A New student</p>
<form method="post">
<p>Student ID:
<input type="text" name="s_ID"></p>
<p>Name:
<input type="text" name="Name"></p>
<p>Address:
<input type="text" name="Address"></p>
<p>Contact:
<input type="text" name="Contact"></p>
<p>Date of Birth:
<input type="date" name="doB"></p>
<p>Date of Joining:
<input type="date" name="doJ"></p>
<p>Paswword:
<input type="text" name="password"></p>
<p>Current Class:
<input type="text" name="current_Class"></p>
<p>Parent ID:
<input type="text" name="p_ID"></p>
<p>Email:
<input type="text" name="email_Address"></p>
<p>Upload Student Image:
<input type="file" name="img_student" enctype="multipart/form-data"></p>
<p><input type="submit" value="Add New"/>
<a href="index.php">Cancel</a></p>
</form>
