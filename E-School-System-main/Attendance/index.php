...
<body>
<h1 style="  position: absolute; left: 630px; top: 0px;">Attendance </h1>
<form method="post">
    ...
    <div class="container">
       ...
       <?php
       if($_SESSION['user_type'] == "admin" OR $_SESSION['user_type'] == "teacher"){
        ?>
        <div class="row">
        <form method="post">
                    ...
                    <a href="add.php" style="color : white;">Add New Attendance Record</a></button>
<?php } ?>                 
 <br>
...
elseif(isset($_POST['Filter']) AND !isset($_POST['Clear'])){
  $s_ID2 = $_POST['s_ID2'];
  $class = $_POST['class'];
  $stmt = $pdo->prepare("SELECT *, student.Name, student.current_Class FROM attendance
                          INNER JOIN student ON attendance.s_ID = student.s_ID
                          WHERE attendance.s_ID = ? OR student.current_Class = ?");
...
elseif($_SESSION['user_type'] == "student" OR $_SESSION['user_type'] == "parent"){
  $stmt = $pdo->query("SELECT *, student.Name, student.current_Class FROM attendance
                      INNER JOIN student ON attendance.s_ID = student.s_ID WHERE attendance.s_ID = ".$_SESSION['id']." OR student.p_ID = ".$_SESSION['id']." ");
}
...
?>
  <div style=" position: absolute; left : 10px; top : 220px; height: 400px; overflow-x: auto;">
...
<?php
    ...
    if($_SESSION['user_type']== "admin" OR $_SESSION['user_type'] == "teacher"){
    echo('<a href="edit.php?id='.$row['id'].'" style="color : green; ">Edit</a>');?></span><span>   /  
           </span><span><?php echo('<a href="delete.php?id='.$row['id'].'" style="color : red;">Delete</a>');}?></span></td>
       <?php
    echo  ("</tr>
      <tr>\n");}?>
    </table>
