...
if(isset($_POST['Filter']) AND !isset($_POST['Clear'])){
    ...
    $stmt = $pdo->prepare("SELECT *, student.Name, student.current_Class FROM marks 
                        INNER JOIN student ON marks.s_ID = student.s_ID WHERE marks.s_ID = ? OR student.current_Class = ?");
                        $stmt->execute([$s_ID2, $class]);
}

else{
    $stmt = $pdo->query("SELECT *, student.Name, student.current_Class FROM marks INNER JOIN student ON marks.s_ID = student.s_ID ORDER BY marks.s_ID");
}
?>
...
<?php
    ...
    <td style="text-align: center;"><span><?php echo('<a href="edit.php?id='.$row['id'].'" style="color : green; ">Edit</a>');?></span><span>   /  
