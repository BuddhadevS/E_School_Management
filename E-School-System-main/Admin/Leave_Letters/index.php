...
<body>
<h1 style="  position: absolute; left: 630px; top: 0px;">Leave Letters</h1>
<form method="post">
    ...
    <div class="container">
        ...
        <form method="post">
            <?php if($_SESSION['user_type']== "admin" || $_SESSION['user_type']== "teacher"){ ?>
                    <label style="position: absolute; left: -100px; top: 35px;">Filter by User ID:</label>
                    <input type="text" name="user_ID" style="position: absolute; left: 150px; top: 30px;" >
    ...
    <button class="btn" style="background-color : #4CAF50;position: absolute; left: 25px; top: 90px; width : 150px;">
                    <a href="add.php" style="color : white;">Add New Marks</a></button>
...
if(isset($_POST['Filter']) AND !isset($_POST['Clear'])){
    $id = $_POST['user_ID'];
    $stmt = $pdo->prepare("SELECT * FROM leave_letter WHERE user_ID = ? ORDER BY n_date DESC");
                        $stmt->execute([$id]);
}

elseif($_SESSION['user_type']== "student" || $_SESSION['user_type']== "parent"){
    $id = $_SESSION['user_name'];
    $stmt = $pdo->prepare("SELECT * FROM leave_letter INNER JOIN student 
                        ON leave_letter.user_ID = student.s_ID 
                        WHERE leave_letter.user_ID = ? OR student.p_ID = ? ORDER BY n_date DESC");
                        $stmt->execute([$id, $id]);
}

else{
    $stmt = $pdo->query("SELECT * FROM leave_letter ORDER BY n_date DESC");
}

?>
  <div style=" position: absolute; left : 10px; top : 150px; overflow-x: auto;">
...
<?php
    while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
     ...
     <tr>
        ...
        <td><?php echo(htmlentities($row['n_date'])); ?></td>
        <td><?php echo(htmlentities($row['letter']));?></td>
