...
if ( isset($_POST['delete']) && isset($_POST['s_ID']) ) {
    $sql = "DELETE FROM library WHERE s_ID = :zip";
...
$stmt = $pdo->prepare("SELECT s_ID FROM library where s_ID = :xyz");
...
?>
<p>Confirm: Deleting Record of Student ID :  <?= htmlentities($row['s_ID']) ?></p>
