...
<body>
<h1 style="  position: absolute; left: 630px; top: 0px;">Teacher Record </h1>
<form method="post">
    ...
    <div class="container">
        ...
        <div class="row">
        <form method="post">
                    <label style="position: absolute; left: -100px; top: 35px;">Filter by Teacher ID:</label>
                    <input type="text" name="t_ID2" style="position: absolute; left: 150px; top: 30px;" >
                    ...
                    <a href="add.php" style="color : white;">Add New Teacher</a></button>
...
if(isset($_POST['Filter']) AND !isset($_POST['Clear'])){
    $t_ID2 = $_POST['t_ID2'];
    $stmt = $pdo->prepare("SELECT * FROM teachers WHERE t_ID= ?");
                        $stmt->execute([$t_ID2]);  
    }
else{
    $stmt = $pdo->query("SELECT * FROM teachers ORDER BY t_ID");
}
?>
...
<?php
    while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
      ?>
     <tr>
        <td><?php echo(htmlentities($row['t_ID'])); ?></td>
    ...
    <td><?php echo '<img src="data:image/jpeg;base64,'.base64_encode($row['img_teacher']).' " width="38" height="30"/>'; ?></td>


    <td style="text-align: center;"><span><?php
    echo('<a href="edit.php?t_ID='.$row['t_ID'].'" style="color : green; ">Edit</a>');?></span><span>   /  
           </span><span><?php echo('<a href="delete.php?t_ID='.$row['t_ID'].'" style="color : red;">Delete</a>');?></span></td>
       <?php
    echo  ("</tr>
      <tr>\n");}?>
    </table>
