...
<head>
    ...
    <title>Libary Book Record</title>
</head>
<style>
  html{
...
<body>
<h1 style="  position: absolute; left: 630px; top: 0px;">Library Book Record </h1>
<form method="post">
    ...
    <div class="container">
        ...
        <div class="row">
        <?php if($_SESSION['user_type'] == "admin" OR $_SESSION['user_type'] == "library" ){?>
        <form method="post">
                    ...
                    <a href="add.php" style="color : white;">Add New Book Record</a></button>
<?php } ?>
 <br>
 <?php
...
if(isset($_POST['Filter']) AND !isset($_POST['Clear'])){
    $s_ID2 = $_POST['s_ID2'];
    $stmt = $pdo->prepare("SELECT *, student.Name FROM library 
                        INNER JOIN student ON library.s_ID = student.s_ID WHERE library.s_ID = ?");
                        $stmt->execute([$s_ID2]);   
        }

// checking if user is student or parents

else if($_SESSION['user_type'] == "student" OR $_SESSION['user_type'] == "parent"){
    $stmt = $pdo->prepare("SELECT *, student.Name FROM library 
                        INNER JOIN student ON library.s_ID = student.s_ID WHERE library.s_ID = ? OR student.p_ID = ? OR student.s_ID =?");
                        $stmt->execute([$_SESSION['user_name'], $_SESSION['user_name'], $_SESSION['user_name']]);  
        }

// if user id admin or library staff

else{
    $stmt = $pdo->query("SELECT *, student.Name FROM library INNER JOIN student ON library.s_ID = student.s_ID");
}
?>
...
<?php
    ...
    <td><?php echo(htmlentities($row['Borrow_Date']));?></td>
    <td><?php echo(htmlentities($row['Return_Date'])); ?></td>
